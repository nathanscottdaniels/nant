...
// Original NAnt Copyright (C) 2001-2006 Gerry Shaw
//
...
namespace NAnt.VSNet {
    internal class CSharpProjectProvider : IProjectBuildProvider {
        public int IsSupported(string projectExt, XmlElement xmlDefinition) {
            if (projectExt == ".csproj")
                return 5;
            if (CSharpProject.IsSupported(xmlDefinition))
                return 10;
            return 0;
        }
        ...
        public ProjectBase GetInstance(SolutionBase solution, string projectPath, XmlElement xmlDefinition, SolutionTask solutionTask, TempFileCollection tfc, GacCache gacCache, ReferencesResolver refResolver, DirectoryInfo outputDir) {
            return new CSharpProject(solution, projectPath, xmlDefinition, solutionTask, tfc, gacCache, refResolver, outputDir);
        }
