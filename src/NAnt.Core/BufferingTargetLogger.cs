...
namespace NAnt.Core
{
    ...
    internal class BufferingTargetLogger : ITargetLogger, IDisposable
    {
        ...
        /// <param name="messageLevel">The <see cref="Level" /> to log at.</param>
        /// <param name="message">The message to log.</param>
        public virtual void Log(Task task, Stopwatch stopwatch, Level messageLevel, string message)
        {
            this.disablingLock.AcquireReaderLock(1000);

            if (disabled)
            {
                this.DestinationLogger.Log(task, stopwatch, messageLevel, message);
            }
            else
            {
                this.LoggingQueue.Enqueue(() => { this.DestinationLogger.Log(task, stopwatch, messageLevel, message); });
            }
        ...
        /// <param name="message">The message to log.</param>
        public virtual void Log(Target target, Stopwatch stopwatch, Level messageLevel, string message)
        {
            this.disablingLock.AcquireReaderLock(1000);

            if (disabled)
            {
                this.DestinationLogger.Log(target, stopwatch, messageLevel, message);
            }
            else
            {
                this.LoggingQueue.Enqueue(() => { this.DestinationLogger.Log(target, stopwatch, messageLevel, message); });
            }
        ...
        public void OnTargetLoggingStarted(object sender, TargetBuildEventArgs e)
        {
            ...
            if (disabled)
            {
                this.DestinationLogger.OnTargetLoggingStarted(sender, e);
            }
            else
            {
                this.LoggingQueue.Enqueue(() => { this.DestinationLogger.OnTargetLoggingStarted(sender, e); });
            }
        ...
        public void OnTaskLoggingStarted(object sender, TaskBuildEventArgs e)
        {
            ...
            if (disabled)
            {
                this.DestinationLogger.OnTaskLoggingStarted(sender, e);
            }
            else
            {
                this.LoggingQueue.Enqueue(() => { this.DestinationLogger.OnTaskLoggingStarted(sender, e); });
            }
